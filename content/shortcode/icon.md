...
Affiche une icône comme `{{% icon cloud %}}`

Cela utilise :

* la bibiothèque glyphicons (bootstrap).[plus d'infos ici](http://getbootstrap.com/components/#glyphicons)
* la bibliothèque fontawesome [plus d'infos ici](http://fontawesome.io/)

Ce shortcode `icon` affichera une icône dans votre page.

## Usage

| Paramètre | Par défaut | Description |
|:--|:--|:--|
| nom | **requis** | nom de l'icône (voir en-dessous) |
| size | none | size of icon, medium, xx-small, x-small, small, large, x-large, xx-large, 11px, 2em, 20%.... |

{{%alert info%}}
**Trucs :**
Ne réglera que le nom comme argument fonctionne aussi : `{{</*icon film*/>}}` au lieu de `{{</*icon name="film"*/>}}`
{{%/alert%}}
